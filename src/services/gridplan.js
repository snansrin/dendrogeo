...
function polyArea(rings){
 let t=0;
 for(const ring of rings){if(!ring||ring.length<3)continue;t+=Math.abs(ringGeodesicArea(ring));}
 return t;
}

function projectPoint(lat,lon,refLat){return {x:lon*111320*Math.cos(refLat*Math.PI/180),y:lat*110540};}
function ringBBox(ring,refLat){
 let a=Infinity,b=Infinity,c=-Infinity,d=-Infinity;
 for(const p of ring){const q=projectPoint(p[0],p[1],refLat);if(q.x<a)a=q.x;if(q.y<b)b=q.y;if(q.x>c)c=q.x;if(q.y>d)d=q.y;}
...
function orientation(a,b,c){const v=(b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x);if(Math.abs(v)<1e-9)return 0;return v>0?1:2;}
...
function segmentsIntersect(a,b,c,d){
 ...
 if(o1===0&&onSegment(a,b,c))return true;if(o2===0&&onSegment(a,b,d))return true;
 if(o3===0&&onSegment(c,d,a))return true;if(o4===0&&onSegment(c,d,b))return true;
 return false;
}
function rectCorners(r){return [{x:r.minX,y:r.minY},{x:r.maxX,y:r.minY},{x:r.maxX,y:r.maxY},{x:r.minX,y:r.maxY}];}
...
async function queryPark(lat,lon,radius=1200){
 ...
 const cands=[];
 for(const el of parkData.elements){const rings=extractRings(el);if(!rings||!rings.length)continue;cands.push({rings,name:(el.tags&&el.tags.name)||null,area:polyArea(rings)});}
 ...
 WATER_RINGS=[];WATER_LINES=[];
 let minLat=90,maxLat=-90,minLon=180,maxLon=-180;
 park.rings.forEach(r=>r.forEach(p=>{if(p[0]<minLat)minLat=p[0];if(p[0]>maxLat)maxLat=p[0];if(p[1]<minLon)minLon=p[1];if(p[1]>maxLon)maxLon=p[1];}));
 const pad=0.0003;
 const bbox=`${minLat-pad},${minLon-pad},${maxLat+pad},${maxLon+pad}`;
 const q2=`[out:json][timeout:60];(way["natural"="water"](${bbox});relation["natural"="water"](${bbox}););out geom;`;
 for(const url of OVERPASS_URLS){
  try{
   ...
   for(const el of (json.elements||[])){
    ...
    if(el.type==="relation"){const r=extractRings(el);if(r)WATER_RINGS.push(...r);continue;}
...
async function queryDetailedCoverage(){
 ...
 let minLat=90,maxLat=-90,minLon=180,maxLon=-180;
 PARK_POLY.forEach(r=>r.forEach(p=>{if(p[0]<minLat)minLat=p[0];if(p[0]>maxLat)maxLat=p[0];if(p[1]<minLon)minLon=p[1];if(p[1]>maxLon)maxLon=p[1];}));
 const pad=0.0003;
 const bbox=`${minLat-pad},${minLon-pad},${maxLat+pad},${maxLon+pad}`;
...
function ringTouchesPark(ring,parkRings,pb){
 ...
 let inCount=0;for(const p of ring)if(pointInPark(p[0],p[1],parkRings))inCount++;
...
function lineTouchesPark(line,parkRings,pb){
 ...
 if(b<pb.minLat||a>pb.maxLat||d<pb.minLon||c>pb.maxLon)return false;
 let inCount=0;for(const p of line)if(pointInPark(p[0],p[1],parkRings))inCount++;
...
function toggleParkMode(){
 ...
 $("parkModeHint").textContent=PARK_MODE?"Şimdi parkın içine tıkla.":"Açınca parka tıkla.";
 bindParkClick();
...
function drawPark(park){
 ...
 const alt=PARK_CANDS.length>1?`<select id="parkAlt" onchange="switchPark(+this.value)" style="font-size:.8rem;padding:4px 8px;border-radius:6px;border:1px solid var(--line)">`+PARK_CANDS.map((c,i)=>`<option value="${i}"${c===park?" selected":""}>${esc(c.name||"Alan "+(i+1))} · ${(c.area/10000).toFixed(1)} ha</option>`).join("")+`</select>`:"";
...
function clearPark(){
 ...
 PARK_POLY=null;IMP_RINGS=[];IMP_LINES=[];GRID_BLOCK_LINES=[];LANDCOVER=null;
}
...
async function buildGrid(){
 ...
 for(let rI=0;;rI++){
  const s0=minLat+rI*dLat,s1=s0+dLat;if(s0>=maxLat)break;
  for(let cI=0;;cI++){
 ...
 (data||[]).forEach(m=>{const cell=cellMap[Math.floor((m.lat-minLat)/dLat)+"_"+Math.floor((m.lon-minLon)/dLon)];if(cell)cell.n++;});
...
function updateGridSummary(g,r0){
 ...
 $("gridSummary").innerHTML=
  ...
  `<button class="btn sm ghost" onclick="downloadWaypointsCSV()">📥 WP CSV</button></div>`;
}
function toggleCellSelection(cellId,rect){
 ...
 let g=0,r0=0;GRID_CELLS.forEach(c=>{if(c.n===0)r0++;else g++;});updateGridSummary(g,r0);
}
function clearCellSelection(){
 SELECTED_CELLS.clear();
 if(GRID_LAYER)GRID_LAYER.eachLayer(l=>{
  if(l.setStyle&&l._cellId){const cell=GRID_CELLS.find(c=>c.id===l._cellId);
...
function toggleWpVis(){
 ...
 if(map.hasLayer(WP_AUTO_LAYER)){map.removeLayer(WP_AUTO_LAYER);$("wpVisBtn").textContent="📍 WP: GİZLİ";}
 else{map.addLayer(WP_AUTO_LAYER);$("wpVisBtn").textContent="📍 WP: GÖRÜNÜR";}
}
...
async function createWaypointsFromGrid(mode){
 ...
 let targetCells=mode==="manual"?GRID_CELLS.filter(c=>SELECTED_CELLS.has(c.id)):GRID_CELLS.filter(c=>c.n===0);
 ...
 let next=(mx&&mx.length?mx[0].wp_id:0)+1;const first=next;
 ...
 $("nProject").value=String(pid);loadWaypoints();
 toast("✓ "+rows.length+" waypoint (P"+first+"–P"+(next-1)+")","ok","📍");
 clearCellSelection();
}

function lineLengthM(l){let len=0;for(let i=1;i<l.length;i++){const dy=(l[i][0]-l[i-1][0])*110540,dx=(l[i][1]-l[i-1][1])*111320*Math.cos(l[i][0]*Math.PI/180);len+=Math.sqrt(dx*dx+dy*dy);}return len;}
function downloadBlob(name,mime,text){const b=new Blob([text],{type:mime});const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);}
function downloadGridGeoJSON(){
 if(!GRID_CELLS.length)return toast("Önce grid");
 const fc={type:"FeatureCollection",features:GRID_CELLS.map(c=>({type:"Feature",properties:{id:c.id,olcum:c.n,durum:c.n===0?"bos":"olculmus"},geometry:{type:"Polygon",coordinates:[[[c.w0,c.s0],[c.w1,c.s0],[c.w1,c.s1],[c.w0,c.s1],[c.w0,c.s0]]]}}))};
...
function downloadWaypointsCSV(){
 ...
 downloadBlob("dendrogeo_wp.csv","text/csv",csv);toast("✓ "+rows.length+" WP","ok","📥");
}
...
function densifyLine(pts,stepM){
 ...
 for(let i=0;i<pts.length-1;i++){
  ...
  const dy=(b[0]-a[0])*110540,dx=(b[1]-a[1])*111320*Math.cos(a[0]*Math.PI/180);
  const len=Math.sqrt(dx*dx+dy*dy);const n=Math.max(1,Math.ceil(len/stepM));
...
function buildNodeIndexW(lines){
 const idx={},cs=0.0004;
 lines.forEach(l=>l.pts.forEach(p=>{const k=Math.floor(p[0]/cs)+"_"+Math.floor(p[1]/cs);(idx[k]=idx[k]||[]).push([p[0],p[1],l.w]);}));
 return {idx,cs};
}
function nearLineW(index,lat,lon){
 const cs=index.cs,i0=Math.floor(lat/cs),j0=Math.floor(lon/cs);
 for(let i=i0-1;i<=i0+1;i++)for(let j=j0-1;j<=j0+1;j++){
  const arr=index.idx[i+"_"+j];if(!arr)continue;
  for(const q of arr){const dy=(q[0]-lat)*110540,dx=(q[1]-lon)*111320*Math.cos(lat*Math.PI/180);if(dx*dx+dy*dy<q[2]*q[2])return true;}
 }
 return false;
}
...
async function runLandCoverAnalysis(){
 ...
 let minLat=90,maxLat=-90,minLon=180,maxLon=-180;
 PARK_POLY.forEach(r=>r.forEach(p=>{if(p[0]<minLat)minLat=p[0];if(p[0]>maxLat)maxLat=p[0];if(p[1]<minLon)minLon=p[1];if(p[1]>maxLon)maxLon=p[1];}));
 const stepLat=5/110540,stepLon=5/(111320*Math.cos(((minLat+maxLat)/2)*Math.PI/180));
 ...
 for(let la=minLat;la<=maxLat;la+=stepLat){
  for(let lo=minLon;lo<=maxLon;lo+=stepLon){
   ...
   nPark++;
   let inWater=false;
   for(const r of WATER_RINGS)if(pointInPolygon(la,lo,r)){inWater=true;break;}
   if(inWater){nWater++;continue;}
   let imp=false;
 ...
 let cross=0;
 IMP_RINGS.forEach(r=>{let vin=0;for(const p of r)if(pointInPark(p[0],p[1],PARK_POLY))vin++;cross+=polyArea([r])*(r.length?vin/r.length:0);});
 IMP_LINES.forEach(l=>{let vin=0;for(const p of l.pts)if(pointInPark(p[0],p[1],PARK_POLY))vin++;cross+=lineLengthM(l.pts)*(l.w*2)*(l.pts.length?vin/l.pts.length:0);});
 ...
 if(rep)rep.innerHTML=
  ...
  `Çapraz kontrol: ~${(cross/10000).toFixed(1)} ha sert</div>`;
 toast("✓ Analiz tamam","ok","🌿");
}

async function drawTiles(ctx,bg,minLat,minLon,maxLat,maxLon,scale,ox,oy){
 const urls={osm:(x,y,z)=>`https://tile.openstreetmap.org/${z}/${x}/${y}.png`,sat:(x,y,z)=>`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${z}/${y}/${x}`,topo:(x,y,z)=>`https://a.tile.opentopomap.org/${z}/${x}/${y}.png`};
 ...
 const imgs=[];
 for(let x=x0;x<=x1;x++)for(let y=y0;y<=y1;y++){const img=new Image();img.crossOrigin="anonymous";img.src=urls[bg](x,y,zoom);imgs.push({img,x,y});}
 try{await Promise.all(imgs.map(o=>o.img.decode?o.img.decode():new Promise((res,rej)=>{o.img.onload=res;o.img.onerror=rej;})));}catch(e){return false;}
 for(const o of imgs){
...
async function downloadParkImage(){
 ...
 if(bg!=="vector"){const ok=await drawTiles(mctx,bg,minLat,minLon,maxLat,maxLon,scale,ox,oy);if(!ok){mctx.fillStyle="#ffffff";mctx.fillRect(0,0,W,H);toast("⚠ Tile yüklenemedi","warn");}}
 if(incCover){
  IMP_RINGS.forEach(r=>{if(!r||r.length<3)return;mctx.fillStyle="#ef444444";mctx.strokeStyle="#dc2626";mctx.lineWidth=1;mctx.beginPath();r.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?mctx.moveTo(xy[0],xy[1]):mctx.lineTo(xy[0],xy[1]);});mctx.closePath();mctx.fill();mctx.stroke();});
  IMP_LINES.forEach(l=>{mctx.strokeStyle="#ef444466";mctx.lineWidth=Math.max(1.5,l.w*scale/55660);mctx.beginPath();l.pts.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?mctx.moveTo(xy[0],xy[1]):mctx.lineTo(xy[0],xy[1]);});mctx.stroke();});
  WATER_RINGS.forEach(r=>{if(!r||r.length<3)return;mctx.fillStyle="#3b82f699";mctx.strokeStyle="#1d4ed8";mctx.lineWidth=1.5;mctx.beginPath();r.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?mctx.moveTo(xy[0],xy[1]):mctx.lineTo(xy[0],xy[1]);});mctx.closePath();mctx.fill();mctx.stroke();});
 }
 if(incGrid){GRID_CELLS.forEach(c=>{const a=toXY(c.s0,c.w0),b=toXY(c.s1,c.w1);const col=c.n===0?"#e11d48":"#16a34a";mctx.fillStyle=col+"66";mctx.strokeStyle=col;mctx.lineWidth=1;mctx.fillRect(a[0],a[1],b[0]-a[0],b[1]-a[1]);mctx.strokeRect(a[0],a[1],b[0]-a[0],b[1]-a[1]);});}
 mctx.globalCompositeOperation="destination-in";mctx.beginPath();PARK_POLY.forEach(r=>{r.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?mctx.moveTo(xy[0],xy[1]):mctx.lineTo(xy[0],xy[1]);});mctx.closePath();});mctx.fill();mctx.globalCompositeOperation="source-over";
 ...
 PARK_POLY.forEach(r=>{ctx.beginPath();r.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?ctx.moveTo(xy[0],xy[1]):ctx.lineTo(xy[0],xy[1]);});ctx.closePath();ctx.stroke();});
 ctx.setLineDash([]);
 if(showWp){wpRows.forEach(w=>{const xy=toXY(w.lat,w.lon);ctx.fillStyle="#e11d48";ctx.beginPath();ctx.arc(xy[0],xy[1],5,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#fff";ctx.lineWidth=1.5;ctx.stroke();});}
 ...
 ctx.fillStyle="#fff";ctx.font="bold 24px system-ui";ctx.fillText("🌳 "+name+" — Saha Raporu",24,40);
 const lines=["DendroGeo · Park Raporu","Park: "+name,"Toplam alan: "+haTotal+" ha (geodezik)"];
 if(PARK_REF_HA)lines.push("Referans: "+PARK_REF_HA+" ha (sapma %"+Math.abs(((polyArea(PARK_POLY)/10000-PARK_REF_HA)/PARK_REF_HA)*100).toFixed(1)+")");
 ...
 ctx.fillStyle="#1f2937";ctx.font="13px system-ui";lines.forEach((t,i)=>ctx.fillText(t,W-bw-8,H-bh-4+24*(i+1)));
 const lg=[];
 ...
 ctx.font="13px system-ui";lg.forEach((e,i)=>{const y=90+i*22;ctx.fillStyle=e[0];ctx.fillRect(W-190,y,16,14);ctx.strokeStyle="#333";ctx.strokeRect(W-190,y,16,14);ctx.fillStyle="#1f2937";ctx.fillText(e[1],W-168,y+12);});
 const mPerDeg=111320*Math.cos(((minLat+maxLat)/2)*Math.PI/180);
 const barPx=200*scale/mPerDeg;
 ctx.fillStyle="#1f2937";ctx.fillRect(24,H-36,barPx,8);ctx.font="bold 12px system-ui";ctx.fillText("200 m",24+barPx+8,H-28);
 canvas.toBlob(b=>{const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download="dendrogeo_"+name.replace(/[^a-z0-9_]/gi,"_")+"_rapor.png";a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);toast("✓ PNG indirildi","ok","🖼️");},"image/png");
}

function setRefHa(v){PARK_REF_HA=parseFloat(v);if(!isFinite(PARK_REF_HA))PARK_REF_HA=null;renderRefBadge();}
function renderRefBadge(){
 const el=$("refBadge");if(!el)return;
