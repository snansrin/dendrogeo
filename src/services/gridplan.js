...
async function runLandCoverAnalysis(){
 ...
 toast("🌿 Yeşil/sert/su analizi (tek seferlik)…","info");
 let minLat=90,maxLat=-90,minLon=180,maxLon=-180;
 PARK_POLY.forEach(r=>r.forEach(p=>{
  if(p[0]<minLat)minLat=p[0];if(p[0]>maxLat)maxLat=p[0];
  if(p[1]<minLon)minLon=p[1];if(p[1]>maxLon)maxLon=p[1];
 ...
 for(const url of OVERPASS_URLS){
  try{
   ...
   if(!res.ok)continue;
   const json=await res.json();
   for(const el of (json.elements||[])){const r=extractRings(el);if(r)GREEN.push(...r);}
   break;
  }catch(e){}
 }
 ...
 for(let la=minLat;la<=maxLat;la+=stepLat){
  for(let lo=minLon;lo<=maxLon;lo+=stepLon){
   ...
   let gr=false;
   for(const r of GREEN){if(pointInPolygon(la,lo,r)){gr=true;break;}}
 ...
 if(rep)rep.innerHTML=
  `<b>🌿 Arazi Örtüsü</b> (5m örneklem · yalnızca park içi)<br>`+
  `🟩 Yeşil: <b>${ha(nGreen)} ha</b> (%${pct(nGreen)}) · 🟫 Sert (bina+yol): <b>${ha(nImp)} ha</b> (%${pct(nImp)}) · `+
  `🟦 Su: <b>${ha(nWater)} ha</b> (%${pct(nWater)}) · ⬜ Diğer: <b>${ha(nOther)} ha</b><br>`+
  `<span style="color:var(--mut)">Toplam park: ${ha(nPark)} ha · yol genişlikleri OSM sınıfına göre hesaplandı</span>`;
...
async function downloadParkImage(){
 ...
 if(bg!=="vector"){
  const ok=await drawTiles(ctx,bg,minLat,minLon,maxLat,maxLon,scale,ox,oy);
  if(!ok){ctx.fillStyle="#ffffff";ctx.fillRect(0,0,W,H);toast("⚠ Tile yüklenemedi → vektör arka plan","warn");}
 }

 if(incCover){
  IMP_RINGS.forEach(r=>{
   if(!r||r.length<3)return;
   ctx.fillStyle="#9ca3af55";ctx.strokeStyle="#6b7280";ctx.lineWidth=1;
   ctx.beginPath();
   r.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?ctx.moveTo(xy[0],xy[1]):ctx.lineTo(xy[0],xy[1]);});
   ctx.closePath();ctx.fill();ctx.stroke();
  });
  IMP_LINES.forEach(l=>{
   ctx.strokeStyle="#6b728088";ctx.lineWidth=Math.max(1.5,l.w*scale/111320*2);
   ctx.beginPath();
   l.pts.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?ctx.moveTo(xy[0],xy[1]):ctx.lineTo(xy[0],xy[1]);});
   ctx.stroke();
  });
  WATER_RINGS.forEach(r=>{
   if(!r||r.length<3)return;
   ctx.fillStyle="#60a5fa99";ctx.strokeStyle="#2563eb";ctx.lineWidth=1.5;
   ctx.beginPath();
   r.forEach((p,i)=>{const xy=toXY(p[0],p[1]);i===0?ctx.moveTo(xy[0],xy[1]):ctx.lineTo(xy[0],xy[1]);});
   ctx.closePath();ctx.fill();ctx.stroke();
  });
 ...
 if(incGrid){
  GRID_CELLS.forEach(c=>{
   ...
   ctx.fillStyle=col+"66";ctx.strokeStyle=col;ctx.lineWidth=1;
 ...
 const bw=360,bh=lines.length*24+20;
